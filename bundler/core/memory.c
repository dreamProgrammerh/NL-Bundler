...
void* memAlloc(const usize size) {
    return __builtin_malloc(size);
}

void* memAllocZ(const usize size) {
    void* mem = __builtin_malloc(size);
    __builtin_memset(mem, 0, size);
    return mem;
}

void* memRealloc(void* ptr, const usize size, bool* success) {
    ...
    void* mem = ptr;
    // Use builtin realloc if available
    #if COMPILER_isGCC || COMPILER_isClang
        mem = __builtin_realloc(ptr, size);
    #else
