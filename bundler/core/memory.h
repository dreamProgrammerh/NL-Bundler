...
/**
 * Allocate 'size' bytes memory on heap.
 *
...
/**
 * Allocate clean zeroed 'size' bytes on heap memory.
 *
 * @param size Number of bytes
 * @return Pointer to memory
 */
void* memAllocZ(usize size);

/**
 * Reallocate memory allocated by memAlloc to new size with same data.
 * 
 * @param ptr Old memory
 * @param size New size
 * @param success OUT whether it fail or succeed
 * @return New memory (pointer can change) OR Old one if it fails
 */
void* memRealloc(void* ptr, usize size, bool* success);

/**
 * Free allocated memory by `memAlloc` function.
 *
